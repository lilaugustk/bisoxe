...
function getDeterministicStyle(numStr) {
    ...
    const settings = [
        "parked on a clean modern city street with green trees in the background",
        "parked on a luxury villa driveway with warm lights in the evening",
        "parked on a sleek asphalt road in a premium urban district",
...
const prompt = `A professional, high-end commercial photograph of the rear view of a luxury ${style.color} ${style.car}. The car is ${style.setting}. The car's license plate is mounted in the center and is extremely clear, displaying exactly the text "${number}" with sharp, bold black characters on a white rectangular reflective license plate with a thin black border. The license plate text "${number}" must be perfectly readable, aligned, and look like a genuine license plate. ${dInstruction ? dInstruction + ' ' : ''}The photograph features realistic lighting, beautiful reflections on the polished car paint, a shallow depth of field with a clean blurry background, and a premium cinematic look.`;
